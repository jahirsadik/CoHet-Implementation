...
def batch_from_rllib_to_ptg(
    ...
    # print(f'pos after: {graphs.pos}')
    # for i in range(graphs.pos.shape[0]):
    #     for j in range(graphs.pos.shape[0]):
    #         dist = (graphs.pos[i] - graphs.pos[j]).pow(2).sum().sqrt()
    #         print(f'distance between {i} and {j}: {dist}')
    # print(f'distances: {graphs.edge_attr[:,2]}')
    # print(f'edge_index: {graphs.edge_index}')
    # sys.exit(0)
    # if PRINT_FLAG:
    # print('Printing graphs')
    # print(f'List: {graphs.to_data_list()[0]}')
    # print(f'Num graphs: {graphs.num_graphs}')
    # print(f'Example: {graphs.get_example(0)}')
        # PRINT_FLAG = False
...
class GPPO(TorchModelV2, nn.Module):
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        # for i in range(len(input_dict['obs'])):
